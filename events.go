...
func decodeEvent(payload string) (*Event, error) {
	...
	if len(items) != 3 {
		return nil, fmt.Errorf("[decode event error! wrong item count, payload: %s", payload)
	}
	code, err := strconv.Atoi(items[0])
	if err != nil {
		return nil, fmt.Errorf("decode event error! wrong event code, payload: %s", payload)
	}
	timestamp, err := strconv.ParseInt(items[1], 10, 64)
	if err != nil {
		return nil, fmt.Errorf("decode event error! wrong timestamp, payload: %s", payload)
	}
	count, err := strconv.Atoi(items[2])
	if err != nil {
		return nil, fmt.Errorf("decode event error! wrong msg count, payload: %s", payload)
	}
	return &Event{
		Code:      code,
		Timestamp: timestamp,
