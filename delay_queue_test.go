...
func TestDelayQueue_consume(t *testing.T) {
	...
	queue := NewDelayQueue("test", UseHashTagKey()).
		WithCallback(cb).
...
func TestDelayQueueOnCluster(t *testing.T) {
	...
	queue := NewDelayQueueOnCluster("test", cb).
		WithFetchInterval(time.Millisecond * 50).
		WithMaxConsumeDuration(0).
		WithLogger(log.New(os.Stderr, "[DelayQueue]", log.LstdFlags)).
		WithFetchLimit(2).
...
func TestDelayQueue_StopConsume(t *testing.T) {
	...
	var received int
	queue = NewDelayQueue("test", func(s string) bool {
		received++
		if received == size {
			queue.StopConsume()
...
func TestDelayQueue_AsyncConsume(t *testing.T) {
	...
	var queue *DelayQueue
	var received int
	queue = NewDelayQueue("exampleAsync", func(payload string) bool {
		println(payload)
		received++
		if received == size {
			queue.StopConsume()
...
func TestDelayQueue_Massive_Backlog(t *testing.T) {
	...
	unackCard, err := redisCli.ZCard(q.unAckKey)
	if err != nil {
		t.Error(err)
		return
	}
	...
	retryLen, err := redisCli.LLen(q.retryKey)
	if err != nil {
		t.Error(err)
		return
	}
